...
## 📸 נכסי שיווק — תמונות ופוסטים מאושרים

### פוסט השקה ראשון — אינסטגרם/פייסבוק (10/7/2026)

**קובץ תמונה:** `assets/marketing/launch-post-hero-v4.png`

**קונספט:** שני עולמות בפריים אחד — כאוס מול סדר.
- צד שמאל (חם/amber): מחברת עם כתב יד עברי מבולגן, sticky notes, ניירות מקומטים, טלפון עם 47+ הודעות WhatsApp.
- צד ימין (navy): טלפון PLTO עם ממשק עברי מלא — Pipeline 5 שלבים + כרטיסי לידים.
- גב טלפון כהה במרכז עם לוגו PLTO — מחלק בין שני העולמות.

**צבעי Pipeline בתמונה (חייבים להיות זהים לאפליקציה):**
חדש (אפור) → יצרנו קשר (כחול) → הצעה נשלחה (סגול #7C3AED) → במשא ומתן (כתום) → עסקה סגורה (ירוק)

**פרומפט Gemini ליצירה מחדש (במידת הצורך):**
```
Same concept: two worlds in one frame. Left side (warm amber): messy Hebrew handwritten notebook, scattered sticky notes, crumpled papers, phone with 47+ WhatsApp unread badge. Right side: deep navy (#0F1F3D) background. A dark phone with PLTO logo on its back stands in the center dividing the two worlds. On the right, a second phone (screen-facing) shows Hebrew PLTO app: header "פייפליין לידים", 5 pipeline stages top to bottom: חדש (gray), יצרנו קשר (blue), הצעה נשלחה (purple #7C3AED), במשא ומתן (orange), עסקה סגורה (green). Below: 3 contact cards — דוד כהן, שרה לוי, עידן מזרחי. No coffee cups. No people. No English text. Square 1:1. Cinematic depth of field.
```

**טקסט הפוסט המאושר:**
```
וואטסאפ. אקסל. מחברת.
ועוד ליד שהלך לאיבוד, ועוד לקוח שלא חזרו אליו.

כשהמידע מפוזר, העסק מנהל — במקום שמנהלים את העסק.

הרשימה מוכרת:
◦ ליד שנרשם בוואטסאפ ונקבר מתחת ל-200 הודעות
◦ לקוח שהבטחנו לחזור אליו — ולא חזרנו
◦ תאריך שחשוב לא לפספס, שפתאום הגיע בלי הכנה
◦ יום שנגמר עם תחושה שרצנו — אבל לא ממש ראינו לאן

זו לא בעיה של ניהול זמן.
זו בעיה של כלי עבודה שלא נבנו לאיך שעסק אמיתי עובד.

אז בנינו את PLTO.

לא עוד מערכת מסורבלת שצריך שבועות להבין.
מערכת אחת, בעברית, שעובדת מהנייד — ושבה הכול במקום אחד:

📋 כל הלידים, הסטטוסים, ההיסטוריה — תמיד בהישג יד
📱 עובדת מהטלפון, כי שם העסק באמת קורה
🤖 AI שעוזר לנסח, לתזכר, ולשמור על קשר חם עם לקוחות
🤝 רשת הפניות בין קולגות — כי הקשרים שווים כסף

PLTO נבנתה יחד עם סוכני נדל"ן, עורכי דין ומעצבי פנים.
הקשבנו לכאבים האמיתיים מהשטח — ובנינו לפיהם.

30 יום בחינם. בלי כרטיס אשראי. בלי התחייבות.
רק לראות איך העסק נראה כשהוא מנוהל נכון.

🔗 plto.app

#PLTO #ניהול_לקוחות #נדלן #עורכי_דין #מעצבי_פנים #CRM #עסקים_ישראלים #פרודקטיביות
```

**סגנון לפרסומים עתידיים:**
- פתיחה בכאב אוניברסלי (לא ברשימת פיצ'רים)
- ניטרליות מגדרית מלאה
- ללא מקפים/חצים על כפתורים
- טון: מקצועי אבל אנושי — "מישהו מבין את הבעיה שלי"
- ללא מילים שחוקות: "מהפכה", "העתיד כבר כאן", "הכלי שישנה את המשחק"
