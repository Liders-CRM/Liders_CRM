...
### ✅ חלק א' — קרדיט הפניה→תשלום (מיגרציות 096-097, 099)
1. **`096_retroactive_admin_set_plan.sql`** — תועד רטרואקטיבית `admin_set_plan()`
...
2. **`097_referral_conversion_reward.sql`** — עמודת `tenants.billing_period`
...
3. **`099_lock_down_referral_reward_internals.sql`** — Supabase security
...
### ✅ חלק ב' — בונה עמודים (Page Builder) (מיגרציות 098, 100 + `page.html` חדש)
...
3. **`client_pages`** table + `create_client_page` (authenticated, rate limit
   ...
   `100_create_client_page_by_token.sql` תיקן: ה-RPC קיבל במקור `p_referral_id`,
...
(מיגרציות `101`-`103`, כולן הוחלו על ה-DB החי ואומתו מחדש בטרנזקציות ROLLBACK)

1. **סטאקינג שגוי כשכמה הפניות "ממתינות" נפתחות ביחד** (101) — כשהמפנה עצמו
...
2. **`converted_tenant_id` נשמר שגוי** (102) — הבאג העמוק ביותר: העמודה נשמרה
...
3. **אין תקרה בין מסלול המפנה למסלול המומר** (103) — התגמול חושב תמיד לפי
