...
## ✏️ ניקוי סימנים רובוטיים — כלל קבוע לכל הסשנים (נקבע 5/7/2026, הורחב 10/7/2026)

> **חל איסור מוחלט להשתמש בסימני מקף רגיל (-) ככלי עיצוב**, בכל הקשר — קוד, UI, תוכן שיווקי, **וגם בתשובות השיחה עצמן** (markdown bullets, רשימות, הסברים טכניים).
>
> **חל על:** כפתורים, כותרות, sub-text, תיאורים, toast, placeholders, מסרי שגיאה, מודלים, תבניות WA, **ותשובות AI לממשק ולמשתמש**.
>
> **כיצד להחליף `-` ברשימות בתשובות שיחה:** להשתמש ב-`•` או ברשימה ממוספרת. אסור להשתמש ב-`-` כ-bullet.
>
> | הקשר | כלל |
> |------|------|
> | **כפתורים** | מוחקים לחלוטין, ללא תחליף |
> | **טקסטים שיווקיים** (כותרות, sub-text) | פסיק בלבד. אסור מקף עיצובי |
> | **רשימות בתשובות שיחה** | `•` או ממוספר. אסור `-` |
> | **נתוני UI** (flows, diagrams) | להשמיט לגמרי |
> | **מקף em/en (— ו–)** | מותר כסימן פיסוק לשוני תקני |
> | **מקף דקדוקי עם מילים זרות** (`ב-CRM`, `מה-AI`) | מותר — זה דקדוק, לא עיצוב |
