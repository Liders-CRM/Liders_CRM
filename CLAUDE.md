...
## מה בוצע — סשן 23/7/2026 (ב') — ביקורת סיכון משפטי לקראת שיווק לעו"ד + תיקונים

> לבקשת המשתמש: סבב "Harvey Specter" (ביקורת קשוחה, לא רק בדיקת קוד) על כל מה שיכול
> לחשוף לתביעה, במיוחד לקראת פנייה יזומה לעורכי דין (קהל שנתפס כתובעני/ביקורתי במיוחד).
> ההנחיה המפורשת שהתקבלה: "כל מה שיכול לסכן אותנו משפטית, תוריד" — עדיפות לצמצום
> פיצ'רים/הבטחות על פני השארתם עם סיכון, גם אם המשמעות פחות פונקציונליות לעו"ד.
> **חשוב**: זו סקירה טכנית/מוצרית שביצע Claude, לא ייעוץ משפטי ממקור מוסמך. הסעיף
> הכי משמעותי (עמלות הפניה) דורש עדיין אישור עורך דין אמיתי בעל ידע באתיקה מקצועית,
> לפי הכלל הקבוע ⚠️ בראש הקובץ. המשתמש אישר לפעול לפי ההמלצה כאן כפתרון ביניים.

### 🔴 הממצא המרכזי, טופל: עמלת הפניה חסומה לגמרי כשמעורב עו"ד
כללי האתיקה המקצועית של לשכת עורכי הדין מגבילים חלוקת שכר טרחה/עמלת הפניה בין עו"ד
לגורם שאינו עו"ד. עד לסשן זה, "🔗 הפניה לקולגה" ו"לוח ההזדמנויות" אפשרו עמלה כספית
בכל שילוב תחומים, כולל מול עו"ד נדל"ן, בלי שום אזהרה.
- **מיגרציה `111_block_lawyer_referral_commission.sql`** (הוחלה על ה-DB החי) —
  `_create_lead_referral_core` ו-`publish_opportunity` נבנו מחדש עם חסימה: כל עוד
  המפנה/הצד המקבל **או** תחום היעד שנבחר הוא `realestate_lawyer`, כל ניסיון להעביר
  `commission_type` שונה מ-`none` נכשל עם `commission_not_allowed_lawyer`. אכיפה
  ב-RPC עצמו (לא רק ב-UI), כך שקריאה ישירה ל-REST לא יכולה לעקוף.
- **`index.html`** — שני המודלים (`modal-lead-referral`, `modal-publish-opportunity`)
  מסתירים לגמרי את שדה העמלה ומציגים הערה ("ℹ️ הפניה/הזדמנות בליווי עו"ד נדל"ן
  מתבצעת ללא עמלה, בהתאם לכללי האתיקה המקצועית") כש-`State.tenant.industry` של
  המשתמש הוא `realestate_lawyer`, **או** כשתחום היעד שנבחר הוא עו"ד נדל"ן
  (`LeadReferral._applyCommissionRule`/`OppBoard._applyCommissionRule`).
- **סגירת פרצה נלווית**: הפניה ל"אחר" (בעל מקצוע חיצוני, לא רשום ב-PLTO) דורשת
  עמלה בכל מקרה (זו הדרך היחידה שיש לה"ה תוכן להציג ב-`sign.html`) — אילו הושארה
  זמינה לעו"ד, זה היה יוצר מבוי סתום (ה-UI לא דורש עמלה, ה-DB חוסם אותה). **נבחר
  לחסום את אפשרות "אחר" לגמרי עבור טננט שהתחום שלו הוא עו"ד נדל"ן** (מוסתרת מה-
  `<select>`), במקום לבנות זרימת "קבלה בלי הסכם" חדשה — פחות פיצ'ר, יותר בטוח,
  תואם את ההנחיה המפורשת של המשתמש.
- **`landing.html`** — הדוגמה השיווקית שהציגה עו"ד נדל"ן גובה 15% עמלה על הפניה
  (בדיוק התרחיש שנחסם) הוחלפה בדוגמה ללא עמלה. שתי "בועות" דקורטיביות עם אחוזי
  עמלה (`tri-bubble tb-1`/`tb-4`) הוחלפו בטקסט ניטרלי. משפט המבוא ללוח ההזדמנויות
  שונה מ"לקבל עמלה מוסכמת" ל"עם עמלה מוסכמת **כשרלוונטי**".

### ✅ תוקנו גם: פערים בין ToS/מדיניות פרטיות לבין המצב בפועל בקוד
1. **"ניתן לבטל בכל עת דרך פורטל החיוב"** (`index.html`, מודל תנאי שימוש) — לא היה
   קיים שום פורטל חיוב בקוד (Stripe demo בלבד, Grow טרם חובר). הנוסח שונה לבטל
   בפועל דרך אימייל (`info@plto.app`), המנגנון האמיתי היחיד שקיים כרגע. גם הוסר
   אזכור "Tranzila" שנשאר טעות משריד ישן (הספק בפועל טרם נקבע/מחובר).
2. **"הנתונים נמחקים אוטומטית לצמיתות אחרי 30 יום"** — נבדק מול כל המיגרציות: **אין
   שום cron/מנגנון מחיקה אוטומטית**, לא בקוד ולא כתהליך ידני מתועד. הנוסח (גם
   ב-`index.html` וגם ב-`privacy-policy.html`) שונה לתיאור מדויק: המחיקה קורית
   "כחלק מתהליך סגירת החשבון", לא הבטחת אוטומציה מובטחת. **לא נבנה מנגנון מחיקה
   אוטומטי בפועל בסבב הזה** (שינוי מסוכן/רחב יותר על דאטה בפרודקשן, לא התבקש
   במפורש) — אם רוצים לחזק את ההבטחה המקורית בעתיד, זה ידרוש לבנות job אמיתי.
3. **תיאור משך הסשן לא תואם התנהגות** — `privacy-policy.html` טען "תוקף כל סשן
   מוגבל ל-8 שעות מרגע הכניסה". בפועל (`AuthSession`, `IdleSession`) המכשיר נשאר
   "מהימן" עד 30 יום בין שימושים, ורק חוסר פעילות בפועל בזמן שהאפליקציה פתוחה
   מנתק אחרי 8 שעות. הטקסט תוקן לתאר את שני המנגנונים במדויק.
4. **תוספות ל-ToS** (מודל `modal-privacy` ב-`index.html`): נוספו סעיפי **תקרת
   אחריות** (מוגבלת לסכום ששולם ב-6 חודשים אחרונים, במקום העדר תקרה), **שיפוי**
   (הלקוח משפה את PLTO על תביעות הנובעות משימושו, כולל הסתמכות על תוכן AI
   שלא נבדק), **מקום שיפוט** (בתי המשפט במחוז תל אביב-יפו בלבד), ו**הבהרת אחריות
   AI כללית** (תוכן AI הוא טיוטה/סיוע בלבד, לא ייעוץ מקצועי/משפטי, באחריות
   המשתמש לבדוק ולוודא התאמה לכללי אתיקה לפני שימוש). נמחקה כפילות של סעיף "דין
   חל" ישן וקצר יותר שנשאר משני עריכות קודמות.
5. **`sign.html`** (חתימת הסכם עמלת הפניה, פונה לצד שלישי שאינו לקוח PLTO) —
   הניסוח "נשמרים כראיה משפטית מחייבת" (קביעה משפטית מוחלטת) רוכך ל"יכולים לשמש
   כראיה לזיהוי החותם ולמועד ההסכמה, בכפוף לכל דין". נוסף checkbox נפרד ("קריאת
   ההסכם שלמעלה ומדיניות הפרטיות, ואישור תנאיהם") שחובה לסמן לפני חתימה, וקישור
   ישיר למדיניות הפרטיות, שלא היו קיימים כלל בעמוד הזה קודם.
6. **הבהרת "טיוטת AI" למשתמש עצמו** (לא רק לפרומפט הפנימי) — שלושת מחוללי התוכן
   במסך השיווק (`genOffer`/`genPost`/`genCampaign`) מציגים כעת "טיוטת AI, יש
   לבדוק ולערוך לפני שליחה/פרסום" מתחת לתוכן שנוצר, לצד תווית המיתוג הקיימת.
   (זרימת `SmartWA` כבר כללה הבהרה דומה קודם, לא שונתה.)

### ✏️ תיקון לממצא קודם (אותו יום) — אין חובת ביטול טלפוני
בכתיבה הראשונית של הסבב הזה נרשם ש"חוק הגנת הצרכן (תיקון 32) דורש גם אפשרות
ביטול טלפונית למנוי מתמשך" כפריט פתוח. בבדיקה חוזרת מול המשתמש הסתבר שזו הייתה
ודאות-יתר: עיקר תיקון 32 הוא שהביטול חייב להיות **לפחות באותה קלות כמו ההצטרפות**,
לא שחייב להתקיים ערוץ טלפוני בכל מקרה. הצטרפות ל-PLTO דיגיטלית לגמרי, כך שערוץ
ביטול דיגיטלי (מייל/הודעת תמיכה מתועדת) תואם את רוח הסעיף. הוסר מרשימת "נשאר
לפעולה" ולא ייבנה קו תמיכה טלפוני על סמך זה.

### 📋 נשאר לפעולה (לא טופל בסבב הזה, דורש החלטה/משאב נוסף)
- **אימות משפטי אמיתי** של חסימת עמלת ההפניה לעו"ד (סעיף המרכזי למעלה) — התיקון
  הטכני בוצע לפי הבנה סבירה של הכלל, לא לפי ייעוץ מוסמך. להציג בפני עורך דין.
- מנגנון מחיקת נתונים אוטומטי אמיתי אחרי סגירת חשבון (סעיף 2 למעלה) — רק תואם
  הטקסט למציאות, לא נבנתה האוטומציה בפועל.
- `privacy-policy.html` עדיין לא כולל סעיפי הגבלת אחריות/שיפוי/מקום שיפוט (אלה
  קיימים רק במודל ה-ToS בתוך `index.html`) — אם ירצו לאחד למקור אמת אחד, זה
  שינוי נפרד.
