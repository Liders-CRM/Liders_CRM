...
## 🧲 פילוסופיית גילוי לקוחות, "יחסים לפני מכירה" — כלל קבוע (נקבע 23/7/2026)

> המשתמש קיבל ממודל שפה אחר מסמך פילוסופיה מפורט על גישת מכירה מבוססת יחסים: לפני
> שמציעים מוצר לליד פוטנציאלי, לבנות אמון ולהבין לעומק את הכאבים העסקיים שלו (שאלות
> פתוחות, הקשבה, לא למהר להציג את PLTO), ורק בסוף לחשוף בעדינות ש-PLTO פותרת בדיוק
> את מה שתואר. כל שיחה כזו היא גם מקור מידע שיווקי, לא רק ניסיון מכירה בודד.

### מה הוחלט בפועל (23/7/2026)
- **לא נבנתה שום תשתית טכנית** (לא צ'אט בוט בדף נחיתה, לא חיבור WhatsApp/Instagram
  Business API אוטומטי). המשתמש בחר לדחות בניית אוטומציה עד שיהיה נפח פניות אמיתי
  שמצדיק זאת, בהתאם לעיקרון הקיים "לא בונים רעיון עתידי באופן יזום" (ראה סעיף ניקוי
  קוד ישן, סשן 12/7/2026).
- **התהליך בפועל כרגע ידני לגמרי**: המשתמש עצמו מנהל שיחות היכרות עם לידים פוטנציאליים
  באינסטגרם/פייסבוק (הודעות אישיות, לא בוט, לא הקלטה, שיחה טקסטואלית רגילה בין שני
  אנשים). **אין בזה שום סיכון משפטי/פרטיות** (בשונה מהדוגמה שכבר סומנה למעלה בסעיף
  בדיקת סיכון משפטי, תמלול שיחות טלפון של צד שלישי): זו שיחה גלויה בין המשתמש עצמו
  לבין הצד השני, בלי הקלטה ובלי עיבוד AI על תוכן שהצד השני לא ידע עליו.
- **תפקיד ה-AI (כל סשן עתידי)**: כשהמשתמש שולח צילום מסך או טקסט של תגובת לקוח משיחה
  כזו, המשימה היא לפענח את התוכן, לחלץ ממנו את הכאבים/ההתנגדויות/הניסוח המדויק של
  הלקוח, ולתרגם את זה לכותרות/פוסטים שיווקיים ל-PLTO (העיקרון מהמסמך: המילים של
  הלקוחות עצמם הן חומר הגלם הכי טוב לשיווק, לא ניסוחים כלליים).
- כשיצטבר נפח פניות משמעותי, יש להעריך מחדש אם שווה לבנות תשתית אוטומטית (למשל
  צ'אט בוט בדף נחיתה, ואולי בהמשך חיבור Meta Business API לאינסטגרם/פייסבוק) — לא
  לבנות זאת באופן יזום מראש.
